...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        while (test-->0) {
            ...
            Integer sun[][] = new Integer[a][b];
            for (Integer i[]:sun)Arrays.fill(i,0);
            for (int l = 0; l < a; l++) {
                for (int k = 0; k < b; k++) {
                    sun[l][k] = sc.nextInt(); }
            }
            ...
            for (int i = 0; i < a; i++) {
                boolean found = true;
                for (int j = 0; j < b; j++)if (sun[i][j] != 0){found = false;break;}
                if (found) n[0]++;
            }
            int i=0;
            while (i<b){
            ...
            if (Math.min(n[0],n[1]) % 2 == 1) sb.append(str[0]+"\n");
            else sb.append(str[1]+"\n");
        }
